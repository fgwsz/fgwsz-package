...
void Unpacker::unpack_key(::std::uint8_t& key){
...
void Unpacker::unpack_relative_path_bytes(
    ::std::uint8_t key
    ...
    //字节序辅助结构体
    using number_type=::std::remove_cvref_t<decltype(relative_path_bytes)>;
    ...
    relative_path_bytes=::fgwsz::net_to_host<number_type>(net.data);
}
void Unpacker::unpack_relative_path_string(
    ::std::uint8_t key
    ,::std::uint64_t relative_path_bytes
...
void Unpacker::unpack_content_bytes(
    ::std::uint8_t key
    ...
    //字节序辅助结构体
    using number_type=::std::remove_cvref_t<decltype(content_bytes)>;
    ::fgwsz::EndianHelper<number_type> net;
    ...
    content_bytes=::fgwsz::net_to_host<number_type>(net.data);
}
void Unpacker::unpack_header(Unpacker::Header& header){
    this->unpack_key(header.key);
    this->unpack_relative_path_bytes(header.key,header.relative_path_bytes);
    this->unpack_relative_path_string(
        header.key
        ,header.relative_path_bytes
        ,header.relative_path_string
    );
    this->unpack_content_bytes(header.key,header.content_bytes);
}
void Unpacker::unpack_content(
    ...
    //判断相对路径是否是安全路径
    ::fgwsz::path_assert_is_safe_relative_path(header.relative_path_string);
    //创建文件父目录
    ::std::filesystem::path file_path=::std::filesystem::absolute(
        output_dir_path/header.relative_path_string
    );
    ...
    while(file_count_bytes<header.content_bytes){
        read_bytes=::fgwsz::std_ifstream_read(
            this->package_
            ,&(block[0])
            ,((block_bytes>(header.content_bytes-file_count_bytes)
                ?(header.content_bytes-file_count_bytes)
                :block_bytes))
            ,this->package_path_string_
        );
        this->package_count_bytes_+=read_bytes;
        ...
        for(::std::uint64_t index=0;index<read_bytes;++index){
            block[index]=static_cast<char>(
                static_cast<::std::uint8_t>(block[index])^header.key
            );
        }
        //将分块读取的content写入文件
    ...
    if(file_count_bytes!=header.content_bytes){
...
void Unpacker::list_package(void){
    ...
    while(this->package_count_bytes_<this->package_bytes_){
        ...
        ::fgwsz::cout<<::std::format(
            ...
            "}}\n"
            ,file_id
            ,static_cast<unsigned>(header.key)
            ,header.relative_path_bytes
            ,header.relative_path_string
            ,header.content_bytes
        );
        //文件内容信息跳过阶段
        this->package_.seekg(header.content_bytes,::std::ios::cur);
        if(!this->package_.good()){
            FGWSZ_THROW_WHAT(
                "failed to skip content bytes: "+header.relative_path_string
            );
        }
        this->package_count_bytes_+=header.content_bytes;
        //更新文件id
        ++file_id;
    }
    if(this->package_count_bytes_!=this->package_bytes_){
